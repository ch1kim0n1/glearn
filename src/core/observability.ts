...
export const coreLogger = new LocalLogger('glearn', (process.env.GLEARN_LOG_LEVEL as LogLevel) || 'INFO');
...
export class LocalAuditLogger {
  ...
  constructor(private tool: string) {
    this.auditDir = process.env.GLEARN_AUDIT_DIR || path.join(os.homedir(), `.${tool}`, 'audit');
    this.retentionDays = Number(process.env.GLEARN_LOG_RETENTION_DAYS || '7');
...
export class MetricsRegistry {
  ...
  recordPublicMethod(method: string, durationMs: number, status: 'ok' | 'error'): void {
    this.increment('glearn_public_method_throughput_total', { method, status });
    this.observe('glearn_public_method_latency_ms', durationMs, { method });
    if (status === 'error') this.increment('glearn_public_method_errors_total', { method });
  }

  prometheus(): string {
    const lines: string[] = [
      '# HELP glearn_public_method_throughput_total Total public method calls by method and status.',
      '# TYPE glearn_public_method_throughput_total counter',
    ];
    for (const [key, value] of this.counters) {
      lines.push(`${key} ${value}`);
    }
    lines.push('# HELP glearn_public_method_latency_ms Public method latency quantiles.');
    lines.push('# TYPE glearn_public_method_latency_ms summary');
  ...
  openTelemetry(): Record<string, unknown> {
    return {
      resourceMetrics: [{
        resource: { attributes: [{ key: 'service.name', value: { stringValue: 'glearn' } }] },
        scopeMetrics: [{
          scope: { name: 'glearn.local' },
...
export class GLearnObservability {
  ...
  constructor(private tool = 'glearn') {
    this.logger = new LocalLogger(tool, (process.env.GLEARN_LOG_LEVEL as LogLevel) || 'INFO');
  ...
  async alertOnHealthDrop(score: number, checks: HealthCheckLike[]): Promise<void> {
    const threshold = Number(process.env.GLEARN_HEALTH_ALERT_THRESHOLD || '80');
    const webhook = process.env.GLEARN_HEALTH_WEBHOOK_URL;
